...
# Magnetization plot.
plt.figure()
# Data plotted with coolwarm blue and increased line width.
plt.errorbar(beta_vals, M_arr, yerr=M_err, fmt='o-', capsize=5,
             label='Data', color='#2c7bb6', linewidth=2)
# Vertical line plotted in coolwarm red with increased line width.
plt.axvline(x=beta_inferred_M, color='#d7191c', linestyle='--',
            label=r'$\beta_{inf}$ = %.3f' % beta_inferred_M, linewidth=2)
plt.xlabel(r'$\beta$')
plt.ylabel(r'$\langle |M| \rangle$ (per spin)')
plt.title(r'Magnetization' + "\n" + r'$(\beta_{inf} = %.3f, \beta_{theo} = %.3f)$' %
          (beta_inferred_M, beta_theo))
plt.legend()
plt.grid(True)
...
plt.errorbar(beta_vals, E_arr, yerr=E_err, fmt='o-', capsize=5,
             label='Data', color='#2c7bb6', linewidth=2)
plt.axvline(x=beta_inferred_E, color='#d7191c', linestyle='--',
            label=r'$\beta_{inf}$ = %.3f' % beta_inferred_E, linewidth=2)
plt.xlabel(r'$\beta$')
plt.ylabel(r'$\langle E \rangle$ (per spin)')
plt.title(r'Energy' + "\n" + r'($\beta_{inf}$ = %.3f, $\beta_{theo}$ = %.3f)' %
          (beta_inferred_E, beta_theo))
plt.legend()
plt.grid(True)
...
plt.figure()
plt.plot(beta_vals, chi_arr, 'o-', label='Data', color='#2c7bb6', linewidth=2)
plt.axvline(x=beta_inferred_chi, color='#d7191c', linestyle='--',
            label=r'$\beta_{inf}$ = %.3f' % beta_inferred_chi, linewidth=2)
plt.xlabel(r'$\beta$')
plt.ylabel(r'$\chi$')
...
# Heat capacity plot.
plt.figure()
plt.plot(beta_vals, cv_arr, 'o-', label='Data', color='#2c7bb6', linewidth=2)
plt.axvline(x=beta_inferred_cv, color='#d7191c', linestyle='--',
            label=r'$\beta_{inf}$ = %.3f' % beta_inferred_cv, linewidth=2)
plt.xlabel(r'$\beta$')
plt.ylabel(r'$C_V$')
